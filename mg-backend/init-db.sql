...
CREATE TABLE IF NOT EXISTS members (
    ...
    payment_class VARCHAR(20) DEFAULT 'evenings' CHECK (
        ...
        payment_status VARCHAR(20) DEFAULT 'overdue' CHECK (
            payment_status IN (
                'outstanding_coupon',
                'half_month',
                'morning_45',
...
CREATE INDEX idx_coupons_expiry ON class_coupons (expiry_date)
WHERE
...
ALTER TABLE class_coupons ADD CONSTRAINT check_remaining_valid CHECK (classes_remaining <= total_classes);
